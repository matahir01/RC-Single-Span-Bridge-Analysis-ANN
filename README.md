...
8. Independent verification campaign and calculation reports. **In progress:** reproducible reference runner, explicit acceptance matrix, external-comparison tolerance records, all-case combined-deflection re-search, and structured formula/substitution/result/reference reporting are implemented in the first Stage-8 batch. Independent STAAD/hand-reference evidence is still required before capabilities are promoted from internal-tested to accepted.
