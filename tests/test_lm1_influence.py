...
def test_actual_grillage_udl_cells_select_signed_unfavourable_regions() -> None:
    ...
    assert positive.response == pytest.approx(
        ...
        + sum(
            effect
            for _, effect in positive.cell_effects
            if effect > 0.0
        ),
        abs=1e-7,
    )
    assert negative.response == pytest.approx(
        ...
        + sum(
            effect
            for _, effect in negative.cell_effects
            if effect < 0.0
        ),
        abs=1e-7,
    )
    assert positive.analysis.vertical_equilibrium_residual_kn == pytest.approx(
        0.0,
        abs=1e-6,
    )
    assert negative.analysis.vertical_equilibrium_residual_kn == pytest.approx(
        0.0,
        abs=1e-6,
    )
    ...
    assert all(
        effect > 0
        for bounds, effect in positive.cell_effects
        if bounds in positive.regions
    )
    assert all(
        effect < 0
        for bounds, effect in negative.cell_effects
        if bounds in negative.regions
    )
    points, areas = build_lm1_plan_loads(
        project,
        placement,
        udl_regions=positive.regions,
    )
    assert len(points) == 4
...
def test_influence_envelope_contains_valid_cases_and_covers_full_udl() -> None:
    ...
    assert all(
        abs(case.analysis.vertical_equilibrium_residual_kn) < 1e-6
        for case in new.cases
    )
    combinations = build_eurocode_project_combinations(
        project,
        new,
        sls_factors=EurocodeServiceabilityFactors(0.75, 0.0),
    )
